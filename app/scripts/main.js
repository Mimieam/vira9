...
(function () {
  ...
  var loginElement = querySelector('#modalLogin');
    ...
    var _API = {
      ...
      server: {
        join: function (name, room) {
          serverSocket.send(JSON.stringify({'msgType': 'join', 'data': {'room': room}}));
        },
        leave: function (name, room) {
          serverSocket.send(JSON.stringify({'msgType': 'leave', 'data': {'room': room}}));
        },
        login: function (name) {
...
var Autumn = (function (name, game) {
    ...
    var AI = function (name, UI, socket) {
        ...
        this.moveTo = function (row, col)  {
            _this.timer.start()
            // console.log(_this.name, ' moving to {' + row +',' + col + '}')

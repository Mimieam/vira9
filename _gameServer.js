      ...
      function client_message_handler(server, _socket, data) {
        ...
        if (data) {
          ...
          switch(_data.msgType){
            ...
            case 'play':
              // join room:
              _socket.join(_data.data);
              console.log(_socket.name + ' just joined ' + _data.data);

              break;
            case 'leave':
              // leave a room
              _socket.leave(_data.data);
              console.log(_socket.name + ' just left ' + _data.data);
              break;
            case 'login':
                _socket.name = _data.data;
                _res = print("Hello " + JSON.stringify(_data) + _socket.id);
        ...
        if (_res){
            Log("Sending ", _res);
            _socket.room(_data.data.room).send(_res);
        }
      }
      // ♬ ♫♬
